...
def analyze_with_gemini(raw_metrics, gemini_key, md_context):
    ...
    # 🚨 [해결] 프롬프트에 강력한 '절대 금지(Negative)' 필터를 주입했습니다.
    prompt = f"""
    ...
    2. 공구 진정성: 캡션을 보고 월 공구 횟수, 빌드업 지수, 최근 한 달 판매 목록 추출.
    3. 찐팬 지표 & 구매 시그널: 캡션의 소통 방식과 평균 좋아요/댓글을 바탕으로 찐팬 비율 추정 및 CS 친절도 유추.
    
    4. AI 추천 전략 & 맞춤 제안서 (매우 중요):
       업로드된 [MD 파일 내용]을 100% 흡수하여 비즈니스 제안서를 작성하되, 아래 🚨[절대 금지 규칙]을 무조건 지키세요.
       
       🚨 [절대 금지 규칙 - 위반 시 감점]: 
       MD 파일 내부에 있는 '비용 분담', '수익 쉐어 비율', '5:5', '광고비 부담' 등 금전적인 조건은 절대 제안서(message)에 1글자도 적지 마세요. 오직 '본사 전폭 지원', 'Meta 파트너십 광고 기술 지원'이라는 긍정적인 혜택만 강조하여 통화/미팅을 유도하세요.
       
       - [페인포인트 공략]: 상대방 피드의 한계점(도달 저하 등)을 공감하며 접근.
       - [상품 핵심 스펙]: 가격, 스펙 강점을 불릿 포인트(-)로 나열.
       - [성공 사례 어필]: 성과(레퍼런스)를 수치로 제시.
       - 가독성 극대화 (복붙용).
    ...
    """
    try:
        st.toast("🧠 AI가 가이드라인의 금지어를 엄격히 준수하며 제안서를 작성 중...", icon="⚡")
