...
# -----------------------------------------------------------------------------
# 4. 데이터 수집 & 가공 함수 (Profile Scraper 전용)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 5. 메인 화면 UI (최종 대시보드 레이아웃)
# -----------------------------------------------------------------------------
st.title("💎 CozCoz Partner Miner (Master Board)")
    ...
    if not st.session_state.md_content:
        ...
        if error:
            ...
            if ai_res:
                ...
                # ==========================================
                st.markdown("### 📊 1. 기초 체력 (Basic Health) - 최근 30일 기준")
                
                with st.container(border=True):
                    # 소개글 & 컨택포인트를 위로 배치하여 가독성 극대화
                    st.info(f"**📝 프로필 소개글 (Bio):**\n{metrics['bio']}")
                    ...
                    # 사진 제외, 릴스만 모은 평균 조회수
                    col3.metric("🎬 릴스 평균 조회수", f"{metrics['avg_reels_views']:,}회") 
                    col4.metric("평균 좋아요", f"{metrics['avg_likes']:,}개")
                    col5.metric("평균 댓글", f"{metrics['avg_comments']:,}개")

                # ==========================================
                # 2. 공구 진정성 검증 (Authenticity Check) - 강조됨
                # ==========================================
                st.markdown("<br>", unsafe_allow_html=True)
                st.error("## 🚨 2. 공구 진정성 검증 (Authenticity Check)")
                
                with st.container(border=True):
                    auth = ai_res['authenticity']
                    c_auth1, c_auth2 = st.columns(2)
                    c_auth1.metric("🛒 월 공구 횟수", auth['gonggu_count'])
                    c_auth2.metric("📈 빌드업 지수 (사전작업)", auth['buildup_index'])
                ...
                with st.container(border=True):
                    ...
                    st.caption("※ 드론 로봇(Profile Scraper)의 특성상 본문 캡션의 소통 유도 방식과 전체 반응도를 기반으로 AI가 추정한 지표입니다.")

                # ==========================================
                # 4. [최종] AI 추천 전략 & 자동 제안서
                # ==========================================
                st.markdown("<br>", unsafe_allow_html=True)
                st.markdown("### 🎯 4. [최종] AI 추천 전략 & 맞춤 제안서")
                
                st.info(f"**💡 AI 추천 전략:** {ai_res['strategy']}")
                
                st.markdown("**📨 자동 제안서 (오른쪽 위 📄 버튼을 누르면 즉시 복사됩니다)**")
                # st.code를 사용하면 텍스트 박스 안에 원클릭 복사 버튼이 자동으로 생깁니다.
